//This file is a pseudo-marginal implementation of the multimodal model with tempered proposal under Scenario (a) with fixed beta
...
model {	                     // model for the posterior
  ...
  ratio = num - denom;
  min_lr = min(ratio);
  target += log_sum_exp(ratio-min_lr) + min_lr - log(N) + sum(denom);
}	

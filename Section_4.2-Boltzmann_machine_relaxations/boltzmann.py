...
#-------------------------------------------------------------------
#Pseudo-extended with fixed beta

nParticles = 2
beta = 0.5 #Choose a beta [0.1-0.9]
...
#Fit Stan model
fit = pystan.stan(file="boltzmann_pseudo-extended_fixedBeta.stan", data=dat, iter=iterations, chains=1)
...
weights = np.empty([iterations//2,nParticles])
for j in range(iterations//2):
    weights[j,:] = np.exp(log_f(theta[j],Q,b)*(1-beta))

#Resample the particles \proto weights 
index = np.array([np.random.choice(a=nParticles, size=1, p=w/sum(w)) for w in weights])

pseudoSamplesFixedBeta = np.array([theta[j,index[j].squeeze(),:]  for j in range(iterations//2)])

#-----------------------------------------------------------------
#A Stan implementation of the Graham and Storkey paper https://arxiv.org/abs/1704.03338
#Note that the simulations in the paper are done using the authors' code https://github.com/matt-graham/continuously-tempered-hmc
